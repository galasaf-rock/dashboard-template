...
function processSales(buf) {
  const rows = readSheet(buf)
  const byPeriod = {}     // period → { salesRows, debtEntries }
  ...
  for (const row of rows) {
    ...
    if (!byPeriod[key]) byPeriod[key] = { salesRows: [], debtEntries: {} }
    byPeriod[key].salesRows.push({ amount, debt })

    if (debt !== 0 && cid) {
      if (!byPeriod[key].debtEntries[cid]) byPeriod[key].debtEntries[cid] = { name, delta: 0 }
      byPeriod[key].debtEntries[cid].delta += debt
      if (name) byPeriod[key].debtEntries[cid].name = name
    }
  }

  const periods = {}
  const debt_by_period = {}

  for (const [key, { salesRows, debtEntries }] of Object.entries(byPeriod)) {
    let revenueTotal = 0
    ...
    debt_by_period[key] = debtEntries
  }

  return { periods, debt_by_period }
}

// ── recomputeDebtLedger ───────────────────────────────────────────────────────
// Rebuilds debt_ledger from scratch by summing all per-month debt contributions.
// Calling this after every upload ensures idempotency: re-uploading a month
// replaces its entry in debt_by_month rather than double-counting.
...
function buildChartData(months_history, maxMonths = 12) {
  ...
  const sortedKeys = Object.keys(months_history)
